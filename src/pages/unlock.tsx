...
export function UnlockPage({ onUnlock, onCreate, vaultExists, defaultVault, availableVaults = [], error }: UnlockPageProps) {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
      ...
      <div className="absolute top-4 right-4 flex items-center gap-1 rounded-full border bg-card/80 p-1 shadow-sm backdrop-blur">
        {(
          [
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <Card className="shadow-xl">
          <CardHeader className="text-center">
            <CardTitle className="text-xl">
              {isCreating ? "Créer un coffre" : "Déverrouiller"}
            </CardTitle>
            <CardDescription>
              {isCreating
                ? "Créez un mot de passe maître pour chiffrer vos données"
                : `Entrez le mot de passe pour "${vaultName}"`}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              {isCreating ? (
                <div className="space-y-2">
                  <label className="text-sm font-medium">Nom du coffre</label>
                  <Input
                    value={vaultName}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Coffre</label>
                  <select
                    value={vaultName}
                    onChange={(e) => setVaultName(e.target.value)}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">Mot de passe maître</label>
              ...
              >
                {loading ? (
                  <span className="flex items-center gap-2">
                    ...
                    {isCreating ? "Création..." : "Déverrouillage..."}
                  </span>
                ) : isCreating ? (
                  <span className="flex items-center gap-2">
            ...
            {vaultExists && (
              <div className="mt-4 text-center">
                ...
                >
                  {isCreating ? "Déverrouiller un coffre existant" : "Créer un nouveau coffre"}
                </button>
              </div>
            )}
          </CardContent>
        ...
        <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
          ...
          <span>Chiffrement AES-256 + ChaCha20-Poly1305 | Argon2id</span>
        </div>
      </div>
    </div>
  )
