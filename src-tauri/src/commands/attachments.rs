...
/// with `row_to_attachment` so adding new polymorphic targets (item / order /
/// subscription) only touches one place.
const ATTACHMENT_SELECT_COLUMNS: &str =
    "id, item_id, order_id, subscription_id, original_name, display_name, mime_type, file_path, size_bytes, attachment_type, created_at";
